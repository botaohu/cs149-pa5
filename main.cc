...
int main(int argc, char **argv)
{
  ...
  if (false) 
  {
    ...
    // There should be two GPUs, we're going to run on the second one
    // which should be the compute only one (i.e. the GTX 560)
    if (numDevices != 2)
    {
      fprintf(stderr,"ERROR: There are not two GPUS on this machine!  Are you running in the PUPS cluster?  If not you will have to modify 'main.cc' to ensure that you are using the proper GPU.\n");
      exit(1);
    }
